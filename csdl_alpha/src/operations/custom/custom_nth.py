...
class CustomExplicitOperationBeta(CustomExplicitOperation):
    ...
    def check_deriv_func_type(self, derivative_operation:Union[Callable, type[CustomOperation]], *args, **kwargs):
        ...
        return call_deriv_func

    def declare_vjp_function(self, vjp_operation:Union[Callable, type[CustomOperation]], *args, **kwargs):
        """Declare a custom operation (or function) that computes the vector*derivatives of this custom operation.
        
        Parameters
        ----------
        derivative_operation : Union[Callable, type[CustomOperation]]
        """
        call_vjp_func = self.check_deriv_func_type(vjp_operation, *args, **kwargs)
        ...
        def vjp_func(cotangents:VarTangents, inputs:list[Variable], outputs:list[Variable]):
            ...
            for i, (input_name, input) in enumerate(self.input_dict.items()):
                ...
                if not cotangents.check(inputs_vjp):
                    continue
                if vjps[input_name] is None:
                    continue
                cotangents.accumulate(inputs_vjp, vjps[input_name])             
    ...
    def declare_derivative_function(self, derivative_operation:Union[Callable, type[CustomOperation]], *args, **kwargs):
        ...
        derivative_operation : Union[Callable, type[CustomOperation]]
        """
        call_deriv_func = self.check_deriv_func_type(derivative_operation, *args, **kwargs)
    ...
    def evaluate_vjp(self, cotangents:VarTangents, *inputs_and_outputs):
        ...
        if self.vjp_func is None:
            raise RuntimeError(f'Derivatives for custom operation {self.info()} has not been set. Use self.declare_derivative_function or self.declare_jvp_function to define derivatives.')
